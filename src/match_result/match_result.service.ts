...
export class MatchResultService {
  ...
  updateMatchResult(matchResultId: number) {
    return this.prisma.matchResult.findFirst({
      where: {
        id:matchResultId,
      },
  ...
  async editScore(
    ...
    const match = await this.findMatch(matchResult.matchId)
    ...
    let awayTeamResult: string;
    let homerankpoint:number;
    let awayrankpoint:number;
    let rank = 1;

    if (!matchResult) throw new NotFoundException('NO matchResult');
    //match creatuser만 경기결과 변경가능
    if (matchResult.homeTeamresult) throw new ForbiddenException('MatchResult Already exists');
    //점수 입력 한번만 가능 수정불가
    ...
    if (matchResult.userId === userId) {
      ...
      if (matchResult.homeTeamScore > matchResult.awayTeamScore) {
        ...
        homerankpoint = 2;
        awayrankpoint = -2;
      } else if (matchResult.homeTeamScore < matchResult.awayTeamScore) {
        homeTeamResult = "패";
        awayTeamResult = "승";
        homerankpoint = -2;
        awayrankpoint = 2;
      ...
      await this.prisma.matchResult.update({
        ...
        await this.prisma.team.update({
          ...
          data: {
            rankpoint: {
              set: hometeam.rankpoint + homerankpoint,
            },
          },
        });
        await this.prisma.team.update({
          ...
          data: {
            rankpoint: {
              set: awayteam.rankpoint + awayrankpoint,
            },
          },
        });
      }
    matchResult = await this.findMatchResult(matchResultId);
    //점수에 따라 경기 결과, rankpoint 변경
    ...
    //임의의 팀 점수 변경할때마다, 모든팀 rankpoint기준으로 재정렬, 순위 할당
    return matchResult;
  }
